...
def bitflip1(f, pos):
    element_val_bin = int(float_to_bin32(f), 2)
    element_val_bin ^= 1 << pos
    element_val_bin = bin(element_val_bin)[2:].zfill(32)
    element_val = bin_to_float32(element_val_bin)
    return element_val
...
def mutate(fiConf, **kwargs):
	...
	for item in ind:
		...
		#print("pos:",pos)
		val_ = bitflip1(val, pos)
		#print("val_:",val_)
		v_[item] = val_
		#print("v_[item]:",v_[item])
	v_ = tf.stack(v_)
	#print("v_:",v_)
	v_ = tf.reshape(v_, elem_shape)
	#print("v_:",v_)
